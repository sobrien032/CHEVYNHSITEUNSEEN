...
// === Utility: Clear button to reset form and previews ===
(function(){
  ...
  clearBtn.addEventListener('click', ()=>{
    form.reset();
    if(typeof resetModels === 'function') resetModels(true);
    ...
    if(submitBtn) submitBtn.textContent = 'Get My Trade Appraisal';
  });
})();

// === Tracking fields: UTM, referrer, landing page ===
(function(){
  const set = (id, val) => { const el = document.getElementById(id); if(el) el.value = val || ''; };
  try{
    const p = new URLSearchParams(location.search);
    set('utmSource',   p.get('utm_source') || p.get('source'));
    ...
    set('landingPage', location.href);
  }catch(e){}
})();

// === Image previews for uploads (max 10MB per file) ===
(function(){
  const MAX_SIZE = 10*1024*1024;
  function wire(inputId, gridId){
    ...
    input.addEventListener('change', ()=>{
      ...
      files.forEach(file => {
        ...
        div.className = 'thumb';
        div.innerHTML = `<img src="${url}" alt=""><div class="meta">${file.name.slice(0,18)}</div>`;
        grid.appendChild(div);
      });
    });
  }
  wire('photoExterior','prevExterior');
  wire('photoInterior','prevInterior');
  wire('photoDash','prevDash');
  wire('photoDamage','prevDamage');
})();
